    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            [
                ...
                'value' => 'sumSp',
                'format' => 'html',
                'footer' => PurchasePlan::find()->select('SUM(outlay) as outlay')
                                                ->where(['and', ['st_id' => Yii::$app->request->queryParams["id"]], ['is_top' => '1']])
                                                ->one()["outlay"],
            ],
            [
                'attribute' => 'p_year',
                'value' => 'p_year',
                'footer' => PurchasePlan::find()->select('SUM(p_year) as p_year')
                                                ->where(['and', ['st_id' => Yii::$app->request->queryParams["id"]], ['is_top' => '1']])
                                                ->one()["p_year"],
            ],
            [
                'attribute' => 'c_year',
                'value' => 'c_year',
                'footer' => PurchasePlan::find()->select('SUM(c_year) as c_year')
                                                ->where(['and', ['st_id' => Yii::$app->request->queryParams["id"]], ['is_top' => '1']])
                                                ->one()["c_year"],
            ],
            'special',
            [
                ...
                'value' => function($data)
                {
                    return PurchasePlan::find()->select('(p_year + c_year + special) as sum')
                                               ->where(['id' => $data["id"]])
                                               ->one()["sum"];
                },
                'footer' => (
                    PurchasePlan::find()->select('SUM(p_year) as sum')
                                        ->where(['and', ['st_id' => Yii::$app->request->queryParams["id"]], ['is_top' => '1']])
                                        ->one()["sum"] +

                    PurchasePlan::find()->select('SUM(c_year) as sum')
                                        ->where(['and', ['st_id' => Yii::$app->request->queryParams["id"]], ['is_top' => '1']])
                                        ->one()["sum"] +

                    PurchasePlan::find()->select('SUM(special) as sum')
                                        ->where(['and', ['st_id' => Yii::$app->request->queryParams["id"]], ['is_top' => '1']])
                                        ->one()["sum"]
                ),
            ],
            ...
            [
                ...
                'value' => function($data)
                {
                    return SchedulePlan::find()->select('SUM(sum) as sum')
                                               ->where(['pp_id' => $data["id"]])
                                               ->one()["sum"];
                },
                'footer' => SchedulePlan::find()
            ...
            [
                ...
                'value' => function($data)
                {
                    ...
                    SchedulePlan::find()->select('SUM(sum_fact) as sum_fact')
                                        ->where(['pp_id' => $data["id"]])
                                        ->one()["sum_fact"];
                },
                'contentOptions' => function ($data)
                {
                    ...
                    (
                        SchedulePlan::find()
                            ->select('SUM(sum) as sum')
                            ->where(['pp_id' => $data["id"]])->one()["sum"] -

                        SchedulePlan::find()
                            ->select('SUM(sum_fact) as sum_fact')
                            ->where(['pp_id' => $data["id"]])->one()["sum_fact"]
                    );
